...
func add_transition(mode_name : String, settings : Dictionary , seconds : float, callback : Callable ) -> void:
	...
	if tree != null:
		...
		tween.parallel().tween_property(self, "color", settings["color"], seconds).set_trans(settings["transition_type"])
		if callback_after_tween_finished.is_valid():
...
func setup() -> void:
	...
	settings_default["scale"] = target.scale / 1.1
	settings_hover["scale"] = target.scale
